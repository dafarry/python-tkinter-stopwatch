...
class digitalclock(tk.Frame):
    def __init__(self, master=None, **kwords):
        ...
        self.buts = dict(zip(buttons, buttons))
        for bname in self.buts:
            bcall = lambda action=self.buts[bname]: self.showtime(action)
            tk.Button(bframe, text=bname, command=bcall).pack(side="left")
        self.lab = tk.Label(self, text="00:00:00", bg=self.cget("bg"))
        self.lab.pack(side="bottom")
        self.showtime(self.buts["Reset"])
    def showtime(self, *args):
        if args:
            act = args[0]
            if act == self.buts["Quit"]:
                self.run = False
                self.quit()
            elif act == self.buts["Reset"]:
                self.timer = 0.0
                self.run = False
                self.lab.config(text = "00:00:00")
            elif act == self.buts["Start"] and self.run == False:
                self.timer = time.time() - self.timer
                self.run = True
            elif act == self.buts["Stop"] and self.run == True:
                self.timer = time.time() - self.timer
                self.run = False
        if self.run == True:
